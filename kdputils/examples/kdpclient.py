...
class KDPClient:
    ...
    def _send_req_and_recv_reply(self, reqpkt):
        kdputils.protocol.send(
            self.sock_reply, (self.kdpserver_host, kdputils.protocol.KDP_REMOTE_PORT), reqpkt, self.seqid, self.sesskey
        )
    ...
    def _reattach(self):
        self.seqid = 0
        replypkt = self._send_req_and_recv_reply(kdputils.requests.kdp_reattach(self.req_reply_port))
        assert replypkt["is_reply"] and replypkt["request"] == KDPRequest.KDP_REATTACH

    def _connect(self):
        replypkt = self._send_req_and_recv_reply(
            kdputils.requests.kdp_connect(self.req_reply_port, self.exc_note_port, b"<o/")
        )
        self.seqid += 1
        assert replypkt["is_reply"] and replypkt["request"] == KDPRequest.KDP_CONNECT

    def get_kernelversion(self):
        replypkt = self._send_req_and_recv_reply(kdputils.requests.kdp_kernelversion())
        assert replypkt["is_reply"] and replypkt["request"] == KDPRequest.KDP_KERNELVERSION
        self.seqid += 1
