...
def kdp_reattach(req_reply_port):
    return dict(
        is_reply=0, request=protocol.KDPRequest.KDP_REATTACH, seq=-1, len=-1, key=-1, req_reply_port=req_reply_port
    )


def kdp_kernelversion():
    return dict(is_reply=0, request=protocol.KDPRequest.KDP_KERNELVERSION, seq=-1, len=-1, key=-1)
