...
def kdp_disconnect():
    return dict(
        is_reply=0x1, request=protocol.KDPRequest.KDP_DISCONNECT, seq=-1, len=-1, key=-1
    )
...
def kdp_readregs(error, regs):
    return dict(
        is_reply=0x1,
        request=protocol.KDPRequest.KDP_READREGS,
        seq=-1,
        len=-1,
        key=-1,
        error=error,
...
def kdp_writeregs(error):
    return dict(
        is_reply=0x1,
        request=protocol.KDPRequest.KDP_WRITEREGS,
        seq=-1,
        len=-1,
        key=-1,
        error=error,
...
def kdp_resumecpus():
    return dict(
        is_reply=0x1, request=protocol.KDPRequest.KDP_RESUMECPUS, seq=-1, len=-1, key=-1
    )


def kdp_reattach():
    return dict(
        is_reply=0x1, request=protocol.KDPRequest.KDP_REATTACH, seq=-1, len=-1, key=-1
    )
...
def kdp_readmem64(error, data):
    return dict(
        is_reply=0x1,
        request=protocol.KDPRequest.KDP_READMEM64,
        seq=-1,
        len=-1,
        key=-1,
        error=error,
...
def kdp_writemem64(error):
    return dict(
        is_reply=0x1,
        request=protocol.KDPRequest.KDP_WRITEMEM64,
        seq=-1,
        len=-1,
        key=-1,
        error=error,
...
def kdp_breakpoint64_set(error):
    return dict(
        is_reply=0x1,
        request=protocol.KDPRequest.KDP_BREAKPOINT64_SET,
        seq=-1,
        len=-1,
        key=-1,
        error=error,
...
def kdp_breakpoint64_remove(error):
    return dict(
        is_reply=0x1,
        request=protocol.KDPRequest.KDP_BREAKPOINT64_REMOVE,
        seq=-1,
        len=-1,
        key=-1,
        error=error,
